...
##
# @description Docknext installer
...
main() {
    ...
    ##
    print_message "\`docknext\` was successfully installed " "success"
...
_install_cli() {
    ...
    cp "$BASE_DIR/../docknext.sh" "$CLI_DIR/docknext"
    chmod u+x "$CLI_DIR/docknext"
...
##
# Add `docknext` as a shell command
#
# @return void
##
_add_shell_command() {
    ...
    print_message "Start add \`docknext\` to commands" "notice"

    ##
    # @note If shell command is already in `PATH`, avoid adding it
    ##
    if [[ "$PATH" =~ $CLI_DIR ]]; then
        print_message "\`docknext\` is already in \`PATH\`" "notice"
    else
        ##
    ...
    print_message "End add \`docknext\` to commands" "notice"
}
